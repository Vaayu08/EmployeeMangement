...
namespace EmployeeMangement.Services
{
    public class AuthService : IAuthService
    {
        ...
        {
            try {
                var addUsers = new List<UserRole>();
                var user = _context.Users.SingleOrDefault(s => s.Id == obj.Id);
                if (user == null)
                {
                ...
                foreach (int role in obj.RoleIds)
                {
                    ...
                    addUsers.Add(userRole);
                }
                _context.UsersRoles.AddRange(addUsers);
                _context.SaveChanges();
                return true;
            ...
            {
                throw new Exception("crediential are not valid");
            }
